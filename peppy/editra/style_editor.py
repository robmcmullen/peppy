...
__svnid__ = "$Id: style_editor.py 53723 2008-05-23 15:53:53Z CJP $"
__revision__ = "$Revision: 53723 $"
...
#                                        style=wx.SUNKEN_BORDER, use_dt=False)
        ...
        self.preview.SetMarginWidth(0, 28)
        self.preview.SetMarginWidth(1, 16)
        self.preview.SetMarginType(1, wx.stc.STC_MARGIN_SYMBOL)
        self.preview.SetMarginWidth(2, 16)
        self.preview.SetMarginType(2, wx.stc.STC_MARGIN_SYMBOL)
        self.preview.SetMarginMask(2, wx.stc.STC_MASK_FOLDERS)
...
#            fname = glob.glob(ed_glob.CONFIG['TEST_DIR'] + fname + ".*")[0]
            fname = glob.glob(os.path.join(util.GetResourceDir('tests'), fname) + ".*")[0]
        except IndexError:
