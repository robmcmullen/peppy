...
class RubberBand:
    ...
    def __init__(self, drawingSurface, aspectRatio=None):
        ...
        drawingSurface.Bind(wx.EVT_MOUSE_EVENTS, self._handleMouseEvents)
        drawingSurface.Bind(wx.EVT_PAINT, self._handleOnPaint)
    ...
    def _handleOnPaint(self, event):
        #print 'paint'
        event.Skip()

    def _isMovingCursor(self):
        """
        Return True if the current cursor is one used to
        mean moving the rubberband.
        """
        return self._currentCursor == wx.CURSOR_HAND

    def _isSizingCursor(self):
        """
        ...
        sizingCursors = [wx.CURSOR_SIZENESW,
                         ...
                         wx.CURSOR_CROSS]
        try:
            sizingCursors.index(self._currentCursor)
            return 1
        except ValueError:
            return 0


    def _handleMouseEvents(self, event):
        """
        ...
        if self.currentBox == None and not event.LeftDown():
            ...
            self._setCursor(wx.CURSOR_CROSS)
            return
        ...
        if event.LeftDown():
            if self.currentBox == None:
                ...
                self.hasLetUp   = 0
            elif self._isSizingCursor():
                # Starting a sizing operation.  Change the origin.
                position = getCursorPosition(x, y, self.currentBox, thickness=self._THICKNESS)
                self.currentBox = self._denormalizeBox(position, self.currentBox)

        elif event.Dragging() and event.LeftIsDown():
            # Use the cursor type to determine operation
            if self._isMovingCursor():
                if self.currentlyMoving or pointInBox(x, y, self.currentBox):
                    ...
                    self._moveTo(x - self.currentlyMoving[0], y - self.currentlyMoving[1])
            elif self._isSizingCursor():
                self._resizeBox(x, y)
    ...
    def _denormalizeBox(self, position, box):
        x, y, w, h = box
        b = box
    ...
    def _resizeBox(self, x, y):
        ...
        if not self.aspectRatio:
            if self._currentCursor == wx.CURSOR_SIZENS:
                x = None
            elif self._currentCursor == wx.CURSOR_SIZEWE:
                y = None
        ...
        newbox = (x0, y0, w, h)
        self._drawAndErase(boxToDraw=normalizeBox(newbox), boxToErase=normalizeBox(self.currentBox))
        self.currentBox = (x0, y0, w, h)

    def _normalizeBox(self):
        """
        Convert any negative measurements in the current
        box to positive, and adjust the origin.
        """
    ...
    def _mouseMoved(self, x, y):
        ...
        if pointOnBox(x, y, self.currentBox, thickness=self._THICKNESS):
            ...
            self._setCursor(wx.CURSOR_HAND)
        else:
            self._setCursor()

    def _setCursor(self, id=None):
        """
        Set the mouse cursor to the given id.
        """
        if self._currentCursor != id:  # Avoid redundant calls
            if id:
                self.drawingSurface.SetCursor(wx.StockCursor(id))
            else:
                self.drawingSurface.SetCursor(wx.NullCursor)
            self._currentCursor = id

    def _moveCenterTo(self, x, y):
        """
    ...
    def _moveTo(self, x, y):
        ...
        self._drawAndErase(boxToDraw=newbox, boxToErase=self.currentBox)
        self.currentBox = newbox

    def _drawAndErase(self, boxToDraw, boxToErase=None):
        """
    ...
    def _dumpMouseEvent(self, event):
    ...
    enabled = property(_isEnabled, _setEnabled, None, 'True if I am responding to mouse events')
