...
class SpectrumPlotProxy(PlotProxy):
    def __init__(self, cubeband):
        PlotProxy.__init__(self)
        self.cubeband=cubeband
    ...
    def getLines(self, x, y):
        ...
        data=numpy.zeros((self.cubeband.cube.bands,2))
        data[:,0]=numpy.arange(1,self.cubeband.cube.bands+1,1)
        data[:,1] = self.cubeband.cube.getSpectra(y,x)
        yaxis=self.cubeband.cube.getUpdatedExtrema()
        if yaxis != self.yaxis:
        ...
        line = plot.PolyLine(data, legend= 'Spectrum', colour='blue')
        return [line]
    
class XProfilePlotProxy(PlotProxy):
    def __init__(self, cubeband):
        PlotProxy.__init__(self)
        self.cubeband=cubeband
        
        self.title='X Profile'
        self.xlabel='sample'
        self.ylabel='value'
    ...
    def getLines(self, x, y):
        #print "XProfilePlotProxy: (%d,%d)" % (x,y)

        profiles=self.cubeband.getHorizontalProfiles(y)

        x=numpy.arange(1,self.cubeband.cube.samples+1,1)
        colorindex=0
        lines=[]
        for values in profiles:
            data=numpy.zeros((self.cubeband.cube.samples,2))
            data[:,0]=x
            data[:,1]=values
            line=plot.PolyLine(data, legend= 'band #?', colour=self.rgblookup[colorindex])
            lines.append(line)
            colorindex+=1
        yaxis=self.cubeband.cube.getUpdatedExtrema()
        if yaxis != self.yaxis:
...
class YProfilePlotProxy(PlotProxy):
    def __init__(self, cubeband):
        PlotProxy.__init__(self)
        self.cubeband=cubeband
        
        self.title='Y Profile'
        self.xlabel='line'
        self.ylabel='value'
    ...
    def getLines(self, x, y):
        # print "YProfilePlotProxy: (%d,%d)" % (x,y)
        
        profiles=self.cubeband.getVerticalProfiles(x)

        x=numpy.arange(1,self.cubeband.cube.lines+1,1)
        colorindex=0
        lines=[]
        for values in profiles:
            data=numpy.zeros((self.cubeband.cube.lines,2))
            data[:,0]=x
            data[:,1]=values
            line=plot.PolyLine(data, legend= 'band #?', colour=self.rgblookup[colorindex])
            lines.append(line)
